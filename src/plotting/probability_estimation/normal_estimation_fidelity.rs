...
pub fn plot_ks_true_normal_estimation_fidelity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    plot_data(
        cdf_diffs,
        "True Normal Distribution KS Test Fidelity",
        "Number of Samples",
        "Fidelity",
        "plots/probability_estimation/ks_true_normal_estimation.png",
    )
}

pub fn plot_ks_false_normal_estimation_fidelity() -> Result<(), Box<dyn std::error::Error>> {
    //! Plots the fidelity of the Kolmogorov-Smirnov test for a false normal distributions
    //!
    //! ## Returns:
    ...
    let mut cdf_diffs: Vec<(f64, f64)> = Vec::new();
    let normal: NormalDistribution = NormalDistribution::new(0.0, 1.0);
    let sample_dist: NormalDistribution = NormalDistribution::new(1.0, 2.0);
    for _ in (0..100).map(|x| 10 * x) {
    ...
    plot_data(
        cdf_diffs,
        "False Normal Distribution KS Test Fidelity",
        "Number of Samples",
        "Fidelity",
        "plots/probability_estimation/ks_false_normal_estimation.png",
    )
}
