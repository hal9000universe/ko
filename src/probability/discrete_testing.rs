...
pub fn estimate_binomial(samples: &Vec<i32>) -> DiscreteProbabilityDistribution<i32> {
    ...
    //!
    //! ## Returns:
    //! * `DiscreteProbabilityDistribution` fitted to `samples`
    let p: f64 = samples.iter().sum::<i32>() as f64 / samples.len() as f64;
    DiscreteProbabilityDistribution::binomial(p)
}

pub fn validate_binomial(
    test_dist: &DiscreteProbabilityDistribution<i32>,
    samples: &Vec<i32>,
) -> bool {
    //! Run the binomial distinction test.
    //!
    //! The binomial distinction test is a statistical test for distinguishing between two `DiscreteProbabilityDistribution<i32>`s
    //! which are binomial.
    //!
    //! ## Arguments:
    //! * `test_dist`: `&DiscreteProbabilityDistribution<i32>`
    ...
    //!
    //! ## Returns:
    //! * `true` if the test distribution is within the confidence interval of the sample distribution
    ...
    let min_p: f64 = (2. * h * n + Z.powi(2)
        - (Z.powi(4) + 4. * h * n * Z.powi(2) - 4. * h.powi(2) * n * Z.powi(2)).sqrt())
        / (2. * (n + Z.powi(2)));
    let max_p: f64 = (2. * h * n
        + Z.powi(2)
        + (Z.powi(4) + 4. * h * n * Z.powi(2) - 4. * h.powi(2) * n * Z.powi(2)).sqrt())
...
pub fn run_binomial_distinction() {
    ...
    let test_dist: DiscreteProbabilityDistribution<i32> =
        DiscreteProbabilityDistribution::binomial(test_p);
    let sample_dist: DiscreteProbabilityDistribution<i32> =
        DiscreteProbabilityDistribution::binomial(sample_p);
    ...
    for n_samples in 10..max_n {
        ...
        println!(
            "n: {} - Binomial distinction test: {}",
            n_samples, distinction
        );
    }
}
